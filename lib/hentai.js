...
const ImageRequest = ({
    url,
    options = {}
}, sizeCallback, progressCallback, endCallback) => {
    ...
    const id = ImageCache.Id(_identity);

    if(ImageCache.Exist(id)) {

        let stream = ImageCache.GetStream(id);
        ...
        let stream = ImageCache.AddStream(id);
