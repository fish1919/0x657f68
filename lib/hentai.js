...
const ImageRequest = (url, options, sizeCallback, progressCallback, endCallback) => {
    ...
    sizeStream
    .on('size', (size) => sizeCallback ? sizeCallback(null, size) : null)
    .on('error', (err) => sizeCallback ? sizeCallback(err) : null);

    wrapProgress(request(url, Object.assign({}, options, {
        encoding: null
    }), endCallback))
    .on('progress', (progress) => progressCallback ? progressCallback(progress) : null)
    .pipe(sizeStream);
