...
const SetCookies = (cookies) => {
    ...
    for(let key in cookies) {
        ...
        CookieJar._jar.setCookieSync(`${ key }=${ value }; path=/; domain=.exhentai.org`, 'http://exhentai.org');
...
const CachedRequest = (url, options, callback) => {
    ...
    if(Cache.Exist(id)) {
        ...
        .on('end', () => callback(null, id, null, Buffer.concat(chunks).toString(options.encoding)));
...
const ImageRequest = (url, options, sizeCallback, progressCallback, endCallback) => {
    ...
    if(Cache.Exist(id)) {
        ...
        .on('end', () => endCallback(null, id, null, Buffer.concat(chunks)));
...
const Detail = ({
    ...
    Flow(function*(cb) {
        ...
        var [err, id, res, body] = yield CachedRequest(url, {
            qs: {
                p: pageIdx
            },
            headers: BuildHeaders({
        ...
        }, (err, id, res, body) => cb(err, id, res, body));
